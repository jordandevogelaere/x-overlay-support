...
class BottomRightSlideNotification extends StatelessWidget {
  ///build notification content
  final WidgetBuilder builder;

  final double progress;

  const BottomRightSlideNotification(
      {Key? key, required this.builder, required this.progress})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return FractionalTranslation(
      translation:
          Offset.lerp(const Offset(1, 0), const Offset(0, 0), progress)!,
      child: builder(context),
    );
  }
}
...
enum NotificationPosition { top, bottom, rightBottom }
